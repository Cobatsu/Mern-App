...
export const  SearchReportModal = React.memo(( { isOpen , close , role , closeModalOnly , setReports , setSubPagesCount , setNotFound, setMainSearchData  }) => {
    ...
    return isOpen ? <SearchModalContainer onSubmit={searchSubmitHandler} >
        ...
        <SearchFields className='SearchFields'>
            ...
            {
                ( role === 'Admin' || role === 'Temsilci' )  ?  <TextField  InputLabelProps={{style:{fontSize:14}}} inputProps={{style:{fontSize:14}}}   style={{width:width<breakPoint ? '90%' : '48%' , margin:'10px 0  0 5px '}} value={searchData['whoseDocument']}  onChange={submitChangeHandler('whoseDocument')}  id="standard-basic" label="Görüşen Kişi"/> : null 
            }
